...
int main(int argc, char *argv[])
{
    if(argc != 3)
    {
        cerr << "Syntax: "<<argv[0]<<" inputfile outputfile"<<endl;
        return -1;
    }
    Transcoder d(argv[1], argv[2]);
	Decoder dec = Decoder(argv[1]);
	ReturnCode error = dec.open();

    error = d.chooseFormat("avi");
    if(!isOk(error))
    {
        cerr << "Format not supported" << endl;
    }
    error = d.chooseVideoCodec(300, 300, 600.0, dec.getFrameRate(), "mpeg4");
    if(!isOk(error))
    {
    ...
    error = d.chooseAudioCodec(22050, 2, 32.0, "libmp3lame");
    if(!isOk(error))
    {
        cerr << "Audio codec not supported" << endl;
    }
