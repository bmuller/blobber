...
FrameRate::FrameRate() : ModInterface("FrameRate") {}

void FrameRate::init(Camarea &area, ProjectionWindow &pw) {
	time(&start);
	count = 0;
}

void FrameRate::update(Camarea &area, ProjectionWindow &pw) {
	...
	if (difftime(now, start) > 3)
	{
		...
		pw.show_message("                              Frame Rate: " + rate + " frames/sec", BLACK);
		...
		pw.show_message("                              Frame Rate: " + rate + " frames/sec");
...
extern "C" {
	ModInterface *get_module() { return new FrameRate(); }; 
}; 
